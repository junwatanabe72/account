...
import { TransactionForJournal, PaymentStatus, JournalGenerationRule, JournalGenerationPattern } from '../../01-types/journalGeneration'
import { JournalData, JournalDetail } from '../../01-types/accounting'
import { AccountService } from '../../03-services/account/AccountService'
...
export class JournalGenerationEngine {
  ...
  // 適用可能なルールを検索
  private findApplicableRule(transaction: TransactionForJournal): JournalGenerationRule | undefined {
  ...
  // 条件にマッチするか判定
  private matchesCondition(transaction: TransactionForJournal, condition: any): boolean {
    ...
    if (condition.tags && condition.tags.length > 0) {
      if (!transaction.tags || !condition.tags.some((tag: string) => transaction.tags?.includes(tag))) {
        return false
      }
    }
  ...
  // 仕訳明細を生成
  private generateJournalDetails(transaction: TransactionForJournal, pattern: JournalGenerationPattern): JournalDetail[] {
  ...
  // 仕訳の摘要を生成
  private generateDescription(transaction: TransactionForJournal): string {
    let description = ''
  ...
  updateRule(ruleId: string, updates: Partial<JournalGenerationRule>) {
    ...
    if (ruleIndex !== -1) {
      this.rules[ruleIndex] = { ...this.rules[ruleIndex], ...updates } as JournalGenerationRule
      // 優先度順に再ソート
      this.rules.sort((a, b) => b.priority - a.priority)
    }
  }
