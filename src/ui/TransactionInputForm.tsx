...
export const TransactionInputForm: React.FC<Props> = ({ engine, onChange }) => {
  ...
  const generateJournalEntry = (trans: TransactionInput) => {
    ...
    if (trans.type === 'income') {
      if (trans.status === 'unpaid') {
        // 未収入金 / 収入科目
        debit = { account: '1141', accountName: '未収入金', amount: trans.amount }
        credit = { account: trans.accountTitle, accountName: getAccountName(trans.accountTitle), amount: trans.amount }
      } else {
        // 現金等 / 収入科目
        debit = { account: trans.paymentAccount || '1111', accountName: getAccountName(trans.paymentAccount || '1111'), amount: trans.amount }
        credit = { account: trans.accountTitle, accountName: getAccountName(trans.accountTitle), amount: trans.amount }
      }
      ...
      if (trans.status === 'unpaid') {
        ...
        credit = { account: '2111', accountName: '未払金', amount: trans.amount }
      } else {
        // 費用科目 / 現金等
        debit = { account: trans.accountTitle, accountName: getAccountName(trans.accountTitle), amount: trans.amount }
        credit = { account: trans.paymentAccount || '1111', accountName: getAccountName(trans.paymentAccount || '1111'), amount: trans.amount }
      }
  ...
  const getAccountOptions = () => {
    ...
    if (activeTab === 'income') {
      ...
      // 費用系の科目（5000番台）
      return accounts.filter(a => a.code.startsWith('5'))
    } else {
      // 資金移動用（現金・預金等）
      return accounts.filter(a => 
        a.code.startsWith('1111') || 
        a.code.startsWith('1121') ||
        a.code.startsWith('1131')
      )
    }
  }

  // 支払口座のオプション
  const paymentAccountOptions = Array.from(engine.accounts.values()).filter(a => 
    a.code === '1111' || // 現金
    a.code === '1121' || // 普通預金
    a.code === '1131'    // 定期預金
  )
            ...
            <div className="mb-3">
              ...
              {activeTab === 'transfer' ? (
                ...
                <select
                  ...
                  onKeyPress={(e) => e.key === 'Enter' && amountRef.current?.focus()}
                >
